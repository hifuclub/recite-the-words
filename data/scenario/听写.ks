...
[eval exp="tf.mode='听写'"][eval exp="tf.词库=0"]
...
[locate x=125 y=526]
[button normal="上一曲" storage=听写.ks target=*上一曲 enterse="page" recthit=true]

[locate x=210 y=524]
[button normal="下一曲" storage=听写.ks target=*下一曲 enterse="page" recthit=true]


[jump target=*暂停中 cond="tf.播放==0"]
[jump target=*播放中 cond="tf.播放==1"]
*暂停中
[locate x=168 y=519]
[button normal="播放" storage=听写.ks target=*播放 enterse="page" recthit=true][jump target=*选择结束]
*播放中
[locate x=164 y=520]
[button normal="暂停" storage=听写.ks target=*暂停 enterse="page" recthit=true]
*选择结束
...
@endnowait
[locate x=550 y=222]
[button normal="确认" storage=听写.ks target=*验证 clickse="l4" enterse="page" recthit=true]
[locate x=620 y=220]
[button normal="换词" storage=听写.ks target=*换词 clickse="l4" enterse="page" recthit=true]
[locate x=550 y=500]
;[button normal="离开" storage=听写.ks target=*end storage=first.ks clickse="l4" enterse="page" recthit=true]



@nowait
[locate x=500 y=10]
[link target=*save storage=听写.ks hint=保存此次成绩到存档]保存成绩[endlink]    
[link target=*load storage=听写.ks hint=读取上次成绩从存档]读取成绩[endlink]
[locate x=500 y=45]
[link target=*选图 storage=听写.ks hint=从背景库中随机选择一张]更换背景[endlink]    
[link target=*关闭背景 storage=听写.ks hint=背景将为黑色]关闭背景[endlink]  
...
[locate x=500 y=207]
;[link hint=播放单词语音 target=*播放语音]语音[endlink]
@endnowait
...
[commit]
[jump storage=听写.ks target=*y cond="tf.英文== tf.sx"]
[jump storage=听写.ks target=*n cond="tf.英文!= tf.sx"]
@s

*y
[save place=备份存档 ask=false]
[jump storage=听写.ks target=*yy cond="f[tf.英文]>1"]
[eval exp="f[tf.中文]=1"]
...
*播放
[jump target=*初回播放 cond="tf.初回播放==0"]
@resumebgm
[eval exp="tf.播放=1"] 
[jump storage=听写.ks target=*界面]
*初回播放
